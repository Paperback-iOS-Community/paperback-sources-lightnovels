 ...
 */
function littleEndianify(size, data) {
    const arr = [];
...
function calculateTextLength(text, font) {
    ...
    for (let c of text.split("")) {
        n += c.charCodeAt(0) - 32 >= 0 && c.charCodeAt(0) - 32 <= 95 ? _fonts_1.fonts[font].font[c.charCodeAt(0) - 32].length / _fonts_1.fonts[font].height : _fonts_1.fonts[font].font[0].length / _fonts_1.fonts[font].height;
    }
    return n;
}
/**
 ...
 */
function writeText(text, page, options) {
    if (typeof options.padding === "number")
        options.padding = { horizontal: options.padding, vertical: options.padding };
    ...
    if (options.constantWidth)
        width = options.width;
    const height = split.length * _fonts_1.fonts[options.font].height + options.padding.vertical * 2;
    let img = [];
    let lineAt = -1;
    for (let i = 0; i < height; i++) {
        ...
        if ((i - options.padding.vertical) % _fonts_1.fonts[options.font].height == 0) {
            lineAt++;
        }
        let letterOn = -1;
        let letter = [];
        ...
        for (let j = 0; j < width; j++) {
            ...
            if (j >= letterBase + letter.length / _fonts_1.fonts[options.font].height) {
                letterOn++;
                letterBase = j;
                let char = split[lineAt].charCodeAt(letterOn) - 32;
                if (Number.isNaN(char) || char < 0 || char >= 95 || char === undefined)
                    char = 0;
                letter = _fonts_1.fonts[options.font].font[char];
            }
            const alpha = letter[((i - options.padding.vertical) - lineAt * _fonts_1.fonts[options.font].height) * (letter.length / _fonts_1.fonts[options.font].height) + j - letterBase];
            if (alpha === 0)
                img[i][j] = splitColor(options.backgroundColor);
            else {
...
function interceptResponse(response, cheerio, settings) {
    if ((response.request.url.includes('ttiparse') || response.request.param?.includes('ttiparse')) && (response.request.url.includes('ttipage') || response.request.param?.includes('ttipage'))) {
        ...
        response.rawData = createRawData(writeImageData(writeText("PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT - PAPERBACK ON BOTTOM - FUCK PAPERBACK - FUCK AWA ESPECIALLY - PAPER SIMPS FOR SHAN - 2 WEEKS MY ASS - PAPER IS SO EMOTIONALLY FUCKING UNSTABLE HE CANT TAKE SOMEONE DOWNVOTING THE CHICK WHO ALREADY HAS A GIRLFRIEND - JIMISWAYTOOEPIC OUT", pageNum, settings)));
